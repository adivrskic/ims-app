                ...
                >
                  <Td>
                    <Link href={`/inventory/${p.id}`} className="block min-w-0">
                      <div className="text-text group-hover:text-[var(--accent)] transition-colors truncate" style={{ fontFamily: "var(--display)", fontSize: "15px", fontWeight: 500 }}>
                        {p.name}
                      </div>
                      {p.manufacturer && (
                  ...
                  <Td>
                    <span className="mono-sm text-text-secondary">{p.barcode}</span>
                    {p.internal_sku && (
                      <div className="mono-sm text-text-dim">SKU {p.internal_sku}</div>
                    )}
                  </Td>
                  <Td>
                    {cat ? (
                  ...
                  <Td align="right">
                    <div className="flex items-center justify-end gap-8">
                      {lowStock && <span className="dot dot-alert" aria-hidden />}
                      <span
                  ...
                  <Td align="right">
                    <time className="mono-sm text-text-dim" dateTime={p.updated_at ?? undefined}>
...
function Td({
  ...
  return (
    <td className={`px-20 py-16 align-middle ${align === "right" ? "text-right" : ""}`}>
      {children}
    </td>
  );
}
