...
- **Inventory** — Product catalog with detail pages, three-field quick add (barcode, name, on hand), spreadsheet import (file or paste, with quantities/locations/costs/suppliers) and CSV export, per-product reorder point / safety stock / lead time.
...
- **Free trial — deploy order.** `supabase/migrations/20260913120000_trial_clock.sql` must be applied to the project **before** the app that reads it is deployed: **1. apply the migration, 2. deploy the app.** Nobody is locked out either way (without the column the app fails open), but a workspace created by the new app before the migration runs comes out grandfathered forever. Ship the app promptly afterwards, too: workspaces created in between are already on the clock.
...
- **Tests** — Vitest unit suite over critical pure logic (`npm test`, 293 tests); no E2E suite yet. Manual coverage is [`docs/QA-TEST-PLAN.md`](docs/QA-TEST-PLAN.md), which is route-complete.
...
- **No E2E suite.** `npm test` covers the pure logic (SSRF guard, redirect safety, RBAC math, API-key scopes, rate limiting, allocation, forecasting, replenishment, FEFO, FIFO layers, Code 128, CSV encoding, onboarding module/priority registry, nav + dashboard resolvers, workspace helpers, the delimited-text parser, import specs, sample catalogs, trial entitlement and its fail-open loader) — 293 tests across 21 files. Anything touching the DB or browser still needs manual verification against the dev server.
- **One entitlement decision.** Whether a workspace may use the product is decided only in `lib/entitlement.ts` (pure, unit-tested): a live Stripe subscription (`active` / `trialing` / `past_due`) → **paid**; `orgs.trial_started_at IS NULL` → **grandfathered** (every workspace that predates trials); otherwise a 7 × 24 h **trial**, then **expired**. Only *expired* loses access, and anything unreadable fails open. `lib/data/entitlement.ts` loads it (service role, request-cached) for every gate: the `(app)` layout → `/trial-ended` (with `components/billing/TrialGuard.tsx` covering client-side navigation and tabs left open), `getActionContext` (the billing actions opt out with `allowExpiredTrial`), the four CSV exports and `/api/v1/*` (both 402). Never gate on `orgs.tier` — owners can edit their own org row. Waiving or extending one workspace's trial is a one-line SQL update; see the migration header.
- **Onboarding drives customization.** The 4-step wizard (`app/onboarding/OnboardingWizard.tsx`, also mounted at `/workspaces/new` for additional workspaces; both entry points share `lib/workspace/provision.ts`) writes `orgs.enabled_modules`, `orgs.priorities` and `warehouses.size_class`; those flow through `lib/modules.ts` → `resolveNav`/`resolveUserNav` (sidenav, precedence `user nav_prefs > org modules > industry > default`) and `resolveDashboard` (`lib/dashboardWidgets.ts`, the overview's visible/ordered widgets). `enabled_modules = null` means "everything on" — every pre-wizard org keeps its exact layout, pinned by the golden parity test in `test/dashboardResolve.test.ts`. Editable later at Settings → Navigation.
- **One import engine.** Products, suppliers and customers import through `lib/import/` (`spec.ts` engine + `specs.ts` per-entity fields with header aliases) over the shared delimited parser in `lib/csv/parse.ts` (RFC 4180 with multi-line cells, comma/tab/semicolon sniffing, BOM-safe). The UI is one client component (`components/import/ImportWorkbench.tsx`): file *or* pasted spreadsheet cells, a dry-run **Check** that shows how columns matched and every row problem, then **Import**. Existing rows are skipped or (opt-in) updated, blank cells never overwrite, and a product `quantity` becomes an on-hand row in the active facility — in the given `SECTION-BAY-LEVEL` or the holding area. Templates come from the same spec (`/api/import-template/<entity>`), so they cannot drift. Add a column by adding a field to the spec; don't hand-parse CSV anywhere else.
- **Sample data is tracked, not tagged.** "Explore with sample data" (`lib/sampleData/`) seeds an industry-appropriate demo set and records every created id on `orgs.sample_data`; "Clear sample data" deletes exactly those ids, in FK order, and refuses (with a plain message) if the customer's own documents reference sample products. The overview subtracts sample counts before judging "has this workspace started?", so the getting-started checklist stays honest while the demo is loaded.
- **One field system.** Every form is built from `components/ui/` primitives — `Input`, `Select`, `Textarea`, `Checkbox`, `FormSection`, `FormActions`, `FormNotice` — over the `FIELDS` block in `globals.css`: a static label above the control, visible placeholder, help/error copy beneath. Don't hand-roll a `field-shell`/`field-label` pair (the label would render inside the box) or a `role="alert"` banner; use the primitive. `FormActions` children go secondary-first, primary LAST.
- **Loading states.** Route `loading.tsx` files use `NautilusLoader` (a drawn logarithmic-spiral shell, `components/ui/NautilusLoader.tsx` + the `NAUTILUS LOADER` block in `globals.css`) for whole-surface waits and content-shaped skeletons for pages. Keep the 12px ring spinner for buttons — the shell is illegible below ~24px.
