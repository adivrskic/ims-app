...
- **No E2E suite.** `npm test` covers the pure logic (SSRF guard, redirect safety, RBAC math, API-key scopes, rate limiting, allocation, forecasting, replenishment, FEFO, FIFO layers, Code 128, CSV encoding, onboarding module/priority registry, nav + dashboard resolvers, workspace helpers, the delimited-text parser, import specs, sample catalogs) — 238 tests across 19 files. Anything touching the DB or browser still needs manual verification against the dev server.
