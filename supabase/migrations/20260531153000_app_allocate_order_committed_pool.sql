-- ATP correctness: the mobile app's pick does NOT decrement locations.quantity
-- (verified against repo hello-world2, app/orders/[id].tsx), and no order
-- transition decrements it either. So on-hand stays "nominal until shipped" and
-- a product is unavailable-to-promise for the GREATER of what's allocated or
-- already picked — not just the unpicked remainder. Otherwise picking an order
-- progressively raises its ATP and oversells the picked-but-unshipped units.
--
-- Update allocate_order's free-pool computation to subtract OTHER open orders'
-- committed units, Σ max(allocated, picked), to match the read-side ATP in
-- lib/data/allocation.ts (getStockAvailability). Greedy allocation still floors
-- each line at its picked qty and caps at requested.
...
begin
  ...
  -- Free pool per product = on-hand minus OTHER open orders' committed units
  -- (max(allocated, picked)). This order's own commitment is excluded so
  -- re-allocation doesn't fight itself.
  for v_prod in
